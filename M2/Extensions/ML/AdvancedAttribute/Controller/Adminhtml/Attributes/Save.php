...
use ML\AdvancedAttribute\Model\Image as ImageModel;
use ML\AdvancedAttribute\Model\Upload;

class Save extends \ML\AdvancedAttribute\Controller\Adminhtml\Banners
{
    protected $_option;
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            try {
                ...
                $_logo = $this->uploadModel->uploadFileAndGetName('logo_attribute', $this->imageModel->getBaseDir(), $data);

                $model->setData($data)
                    ->setImage($_image)
                    ->setIconNormal($_iconNormal)
                    ->setIconHover($_iconHover)
                    ->setLogoAttribute($_logo);
    ...
    {
        $collection = $this->_objectManager->create('ML\AdvancedAttribute\Model\ResourceModel\Option\Collection');
