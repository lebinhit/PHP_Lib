<?php
namespace ML\AdvancedAttribute\Block;

use Magento\Framework\View\Element\Template;
use Magento\Framework\View\Element\Template\Context;
use ML\AdvancedAttribute\Model\Status;

class BannerAttribute extends  \Magento\Framework\View\Element\Template
{

    protected $_template = 'ML_AdvancedAttribute::banner.phtml';
    protected $_itemTemplate = 'ML_AdvancedAttribute::slideritem.phtml';
        ...
        if (
            ...
            if (is_null($this->_bannerAttribute)) {
                ...
                $collection->getSelect()->joinLeft(
                    array('info' => 'ml_attribute_option_banner'),
                    'info.option_id = main_table.option_id',
                    array('image', 'logo_attribute', 'content', 'is_active', 'html_title')
                )->where("info.is_active = " . \ML\AdvancedAttribute\Block\Adminhtml\Options\Edit\Tab\Main::IS_ACTIVE_YES);
    ...
    {
        $modelImg = $this->_objectManager->get('ML\AdvancedAttribute\Model\Image');
    ...
    public function getLogoBrand()
    {
        ...
        if ($collection->getSize() > 0) {
            ...
            if ($_valueImage) {
                $logo = $this->_objectManager->get('ML\Brand\Helper\Image')
